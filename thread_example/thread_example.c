...
static void draw_callback(Canvas* canvas, void* context) {
    UNUSED(context);  // TAMA : 2024-02-29 : TEST
...
static void charcat(char* dest, char* src) {
    int i=0, j=0, m=sizeof(dest)-1, n=sizeof(src)-1;
    while (dest[i]) ++i;
    while (src[j] && i<m && j<n) dest[i++] = src[j++];
    dest[i] = '\0';
}
...
static void configure_and_start_thread(TimeTableThread* timeTableThread) {
    ...
    charcat(thread_name, string_time_table);  // strcat(thread_name, string_time_table); TAMA : 2024-02-29 : TEST
