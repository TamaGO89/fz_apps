...
void configure_text_input_box(AdvancedGUI* app, FuriMutex* state_mutex) {
...
void configure_widget(AdvancedGUI* app, FuriMutex* state_mutex) {
    UNUSED(state_mutex);
...
void configure_view_dispatcher(AdvancedGUI* app, FuriMutex* state_mutex) {
    // Gui
...
void run_flipper_application(AdvancedGUI* app) {
    ...
    configure_text_input_box(app, &app->mutex);
    configure_view_dispatcher(app, &app->mutex);
    configure_widget(app, &app->mutex);
